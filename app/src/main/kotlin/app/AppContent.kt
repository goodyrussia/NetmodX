...
private object MainNavigation {
    ...
    fun navigationItems(): List<NavigationItem> {
        ...
        val log = stringResource(R.string.nav_log)
        val settings = stringResource(R.string.nav_settings)

        return remember(languageMode, proxy, log, settings) {
            listOf(
                NavigationItem(proxy, MiuixIcons.Layers),
                NavigationItem(log, MiuixIcons.File),
        ...
        pageContent = { page ->
            key(languageMode, page) {
                when (page) {
                    ...
                    MainNavigation.LOG_PAGE_INDEX -> CoreLogsPage(padding = padding)
